...
1. [최신 GitHub Release](https://github.com/Bongseop-Kim/env-pilot/releases/latest)에서 `Env-Pilot-<version>.zip`을 받습니다.
2. 압축을 풀고 `env-pilot.app`을 `/Applications`로 옮깁니다.
3. 처음 한 번은 Finder에서 앱을 Control-클릭한 뒤 **열기**를 선택합니다.

현재 배포본은 Apple Developer ID 서명 및 공증을 거치지 않습니다. Sparkle 업데이트 파일은 별도의 EdDSA 키로 서명되지만, macOS는 최초 실행 시 확인 경고를 표시할 수 있습니다.
...
각 Release에는 다음 두 파일이 함께 올라가야 합니다.

- `Env-Pilot-<version>.zip`: 유니버설 앱 번들
...
스크립트는 유니버설 Release 빌드, Sparkle 구성요소의 ad-hoc 서명, zip 생성, EdDSA 서명 appcast 생성, GitHub Release 업로드를 순서대로 수행합니다. Sparkle 개인키는 macOS Keychain의 `com.bongsub.env-pilot` 계정에 보관되며 Repository에는 저장되지 않습니다.

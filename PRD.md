...
- 사용자 요청 없이 앱이 정한 출력 경로로 env 파일 자동 생성
...
사용자는 Variables 탭의 `Env 파일 관리`에서 실제 파일을 명시적으로 생성하거나 이름/상대 경로를 변경하고 삭제할 수 있다. 생성·이동할 경로의 상위 폴더는 이미 존재해야 하며, 삭제 전에는 디스크 파일과 연결된 Variable·Secret이 함께 제거된다는 확인을 받는다.
...
- `Env 파일 관리`에서 새 파일 생성, 선택 파일의 이름/상대 경로 변경, 삭제를 제공한다.
...
- 빈 Repository에는 `.env 파일을 찾지 못했습니다`, `새 .env 파일`, `다시 찾기`를 표시한다.
...
- [ ] `.env` 또는 `.env.*` 파일을 직접 생성하고 이름/상대 경로를 변경하거나 확인 후 삭제할 수 있다.
